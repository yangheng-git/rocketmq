...
public class MQClientInstance {
    ...
    // 索引值， 一般是0 ， 因为客户端实例 一般都是一个进程， 只有一个
    ...
    // 【核心】
    // 核心的一个API实现，它几乎包含了所有服务端 的API, 它的作用就是将MQ业务层的数据 转换为 网络层的 RemotingCommand 对象
    ...
    private void startScheduledTask() {
        ...
        // 定时任务1： 从nameserver 更新客户端本地的路由数据
        // 周期 30秒
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
        ...
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                try {
                    // 清理下线的broker节点， 检查客户端的 路由表， 将路由表中 不包含的addr 清理掉。 如果被清理的brokerName 下的所有的物理节点 都没有了，
                    // 需要将borker 映射数据 删掉。
    ...
    public boolean updateTopicRouteInfoFromNameServer(final String topic) {
         ...
         * @param defaultMQProducer s生产者对象
         *
         *
         * 发送消息路径
         * @param topic 主题
     ...
     *                          <p>
     *                          发送消息路径（特殊情况）
     *                          因为 nameserver 上 没有 topic 的路由数据，所以 拿到的 topicPublishInfo 还是一个 空对象。
     * @param topic             主题
     * @param isDefault         false
     * @param defaultMQProducer s生产者对象
     */
            ...
            if (this.lockNamesrv.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    if (topicRouteData != null) {
                        ...
                        // 对比， 本地路由数据 与 nameserver最新下拉的 路由数据， 是否一致。 如果发生变化。  changed =true;
